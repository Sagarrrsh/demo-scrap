...
- Reliability via outbox: API writes succeed first, and broker publish is retried asynchronously.
...
## Reliability improvement added
- **Transactional outbox** in `user-service` ensures request writes and event enqueue happen atomically before broker publish.
- Added admin-triggered outbox flush endpoint to replay pending events safely.
