      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
                  {priceHistory.map((h, idx) => (
                    <div key={idx} className="p-4 border-2 border-gray-100 rounded-xl hover:border-green-200 transition-colors">
                      ...
                      {h.reason && (
                        <p className="text-sm text-gray-600">{h.reason}</p>
                      )}
                    </div>
                  ))}
                </div>
  ...
  const fetchRequests = async () => {
    ...
    try {
      const url = filter === 'all' 
        ? `${API_BASE}/users/requests`
        : `${API_BASE}/users/requests?status=${filter}`;
              ...
              <div key={cat.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
                ...
                <div className="p-6">
                  ...
                  <div className="flex gap-2">
                    {user && (
                      <button
                        onClick={() => handleCreateRequest(cat)}
                        className="flex-1 py-2 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-lg font-semibold shadow hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300"
                      >
                        Request Pickup
                      </button>
                    )}
                    <button
                      onClick={() => fetchPriceHistory(cat)}
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  {selectedHistoryCategory && (
                    <p className="text-gray-600">{selectedHistoryCategory.name}</p>
                  )}
                </div>
                <button
